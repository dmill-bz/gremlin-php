...
if [ $GREMLINSERVER_VERSION = "3.2.1" -o $GREMLINSERVER_VERSION = "3.1.3" ]; then
...
if ! [ $GREMLINSERVER_VERSION \< "3.3.0" ]; then
  if ! [ $GREMLINSERVER_VERSION \< "3.4.0" ]; then
    TP_CONF_DIR="3.4.x"
  else
    TP_CONF_DIR="3.3.x"
  fi
